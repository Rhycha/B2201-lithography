...
plt.plot(xs, uncharging(xs, A, t, c, b), '--', label="uncharging", color="skyblue")

# 색칠하기 #######################
# 정해진 범위 내에 하늘색으로 면적 칠하기
plt.fill_between(xs, 0, uncharging(xs, A, t, c, b), where=(xs >= min(xs)) & (xs <= max(xs)), facecolor='skyblue', alpha=.8,)
...
Fb = int_func.subs(x,xs[-1]).evalf()
Fa = int_func.subs(x,xs[0]).evalf()

print(Fb-Fa)
plt.text((xs[0]+xs[-1])/2, min(ys[0],ys[-1])/2, str(round(Fb-Fa, 2)), horizontalalignment='center')
...
A, t, c, b = params_c
plt.plot(xs2, charging(xs2, A, t, c, b), '--', label="charging", color="red")

# 색칠하기 #######################
# 정해진 범위 내에 하늘색으로 면적 칠하기
plt.fill_between(xs2, 0, charging(xs2, A, t, c, b), where=(xs2 >= min(xs2)) & (xs2 <= max(xs2)), facecolor='red', alpha=.7,)
...
int_func = sy.integrate(charging_for_sym2(A,t,c,b),x)
Fb = int_func.subs(x,xs2[-1]).evalf()
Fa = int_func.subs(x,xs2[0]).evalf()

print(Fb-Fa)
plt.text((xs2[0]+xs2[-1])/2, min(ys2[0],ys2[-1])/2, str(round(Fb-Fa, 2)), horizontalalignment='center')
...
def transientArea(xs, ys, xs2, ys2):
    ...
    plt.text(x1, meany/2, str(round(area,2)),
             horizontalalignment='center')
    # plt.text((xs2[0] + xs2[-1]) / 2, min(ys2[0], ys2[-1]), str(round(Fb - Fa, 2)), fontsize=14,
    #          horizontalalignment='center')
